...
- **A build has a version and it comes from the commit, not the deploy.** `YYYY.MM.DD.N`, where N is how many commits that day the build contains—computed by `utils/deploy/deploy.sh`, shipped as `APP_VERSION`, and read by `src/version.ts`, which falls back to `dev`. Redeploying the same tree reports the same version because it is the same code. **A new key here has to be added to the allow-list in `deploy.sh` or it reaches no environment**; `APP_VERSION` and `BUILD_SHA` are both on it.
