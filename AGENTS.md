...
- Eight pure client helpers own arithmetic rather than DOM and are `eval`'d by their own tests: `ride-time.js`, `twist.js`, `route-shape.js`, `builder-history.js`, `duration.js`, `alts.js`, `feedback-buffer.js`, `place-query.js`.
...
- **The roadbook prints times shifted by the rider's UTC offset, and it is not fixed.** `days.start_at` is `timestamp with time zone`, so a rider typing 9am Pacific is stored `16:00+00`—correct. The roadbook then renders it with `timeZone: 'UTC'` and prints **4:00 PM**, while the builder's own timeline reads 9:00 AM from the viewer's zone. The same day, two answers, seven hours apart. UTC rendering would be right for a naive `timestamp` and is wrong for a `timestamptz`; the real gap is that nothing stores WHICH zone a day's 9am is in. Fixing it is a decision (viewer's zone / a zone column / naive wall-clock), not a patch. Do not "tidy" the UTC away without making that decision.
