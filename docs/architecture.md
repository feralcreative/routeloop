...
- **A day is one ordered list of points, and `legs[i]` joins `points[i]` to `points[i+1]` whatever kind either end is.** Ziad's call, 2026-08-24: a POI is something the rider will at least ride BY, so it is always part of the route and `kind` says only whether they mean to stop there. Every point created by a rider is a POI, promoted to a stop **by being given a category** or from the row menu—except the first point of a day, which `addPoint()` promotes on the spot and tags `start`. A category is a reason to stop, so choosing one promotes and clearing the last one demotes; the menu items survive for a stop with no reason given. Promotion is a pure flag flip: it routes nothing, rebuilds no leg, and leaves the road identical.
- **There is one index space.** `stopIdx()` and `stopOrdinalAt()` in `builder.js` are gone with the second one; `stopsOf()` survives on both sides only for the things that genuinely count stops rather than points—`rides.stop_count`, the roadbook's numbered rows, the Maps hand-off, and the at-least-one-stop rule. Note what this costs: adding a POI is a Routes request now, because it splits the leg it lands in.
