...
- **`rides`**—`owner_id`, unguessable `slug`, `title`, `description`, `visibility` (public / unlisted / private / friends), `source` (native | imported), `external_url`, the byte columns plus generated `size_bytes`, and the caches `total_miles`, `total_duration_s`, `stop_count`. **`original_stored_at` is when the uploaded file was written**, stamped in the same transaction that writes it, and `updated_at > original_stored_at` is how a download tells a ride that still IS its upload from one rebuilt in the builder since—the same shape as `updated_at > thumb_built_at`. Null means no original was ever stored, which is every native ride; it is read as *current*, never as stale, and every caller checks `hasStored()` first. Backfilled from `created_at` in `drizzle/0023` for rides that already had one, which is exact rather than a guess: the import inserts the row and writes the files in one transaction.
- **`days`**—`ride_id`, `position`, `uid` (the day's durable identity, unique per ride), `subgroup_id` (nullable; null means everyone rides it), `title`, `color` (feeds the legend), `start_at`/`end_at` (nullable), `distance_m`, `duration_s`, the alternates pair `alt_group` (nullable smallint) / `alt_active` (not null, default true), and `route_prefs` (nullable jsonb). A position, not a calendar date: two days may share a date, and an undated ride still has days.
...
- **`ride_members`**—a rider's relationship to a ride: `role` (owner | rider), `perm` (view | comment | suggest | edit), `rsvp` (invited | going | maybe | declined), `invited_by` (`set null`, so losing the inviter's account does not evict everyone they brought). Unique on (ride, rider); `idx_ride_member_rider` is rider-first because the question is always "is this viewer on this ride". Three columns because they are three different questions: who the ride belongs to, what this rider may do to it, and whether they are coming. **`perm`'s member order is not its rank**—`PERM_RANK` in `src/members/policy.ts` is, for the same reason `visibility` cannot be sorted. Existing rows backfilled to `suggest` in `drizzle/0020`, via the column default. **`role = 'owner'` may be held by more than one member** as of 2026-08-28; `rides.owner_id` stays singular and keeps meaning the creator and the quota holder.
- **`ride_comments`**—what a rider said about a ride, or about one point on it. `point_uid` null means the ride itself. **An orphaned comment demotes to ride level rather than being deleted**, which is the opposite of `point_details` and `alt_votes`: those are data about a point, and a comment is a thing a person said. `point_label` is the point's name copied in at post time and never updated—once the point is gone there is nothing left to join to, and the label is all that keeps the comment from being about nothing. `resolved_at` is a timestamp rather than a flag so the record says when. Cascades from `rides` and from `users`.
- **`ride_suggestions`**—a proposed replacement for one day, waiting on the owner. `day_uid` (never an id—days churn), `payload` (the proposed day as jsonb, in the shape the builder's PUT accepts), `base_fingerprint` (what the day looked like when the proposal was made), `resolved_at` + `outcome` (accepted | discarded | withdrawn). **Staleness is derived, never stored**: a suggestion is stale when the target day's fingerprint no longer matches, so nothing sweeps, nothing is invalidated on save, and a day edited and then edited back correctly stops being stale. There is no `pending` or `stale` member of the outcome enum for exactly that reason.
...
- **`user_profiles`**—the FK to `users` *is* the PK, so there is one profile per rider and no surrogate id to keep in sync. Name, address and the geocoded home, the public start place, the sharing toggles and the payment handles. Also the display preferences—`duration_format`, `date_format`, `clock`, `units`, `volume_units`, `theme`, `scheme`, `motion`—and `avoid_places`. None of them is a storage unit: `points.duration_min` stays integer minutes and `routes.start_at` stays a timestamp whatever they say. **A rider may have no row here at all**, so every reader has to have an answer for that—see `toDurationFormat()` in `src/maps/duration.ts` and `dateFormatFor()` in `src/views/prefs.ts`, and note that the write path upserts rather than updates. That lazy creation is load-bearing for `date_format`: with no row, the format falls back to `Accept-Language`, so an upsert saving some *other* preference must seed `date_format` on INSERT and leave it out of the update set—all six handlers in `settings.tsx` do, and a seventh has to. `avoid_places` is free text a rider types (#271) and is stored exactly as typed, never normalized: `parseAvoidList()` in `src/places/avoid.ts` is one pure function away at every read, and normalizing on save would rewrite a rider's own list under them.
...
**`days.route_prefs` is what the day asks of the router, and it is jsonb because the set grows.** `src/maps/route-prefs.ts` owns the shape and the one mapping to Google's `routeModifiers`; the column is nullable with no default, which is what made it safe in a single deploy under the expand/contract rule—null means no preference, every row predating it already meant exactly that, and the release before it never wrote the field. `{}` normalizes to null on the way in, because two spellings of one state make `dayRevision()` disagree with itself and manufacture a save conflict nobody caused. The object is not open: `routePrefsSchema` is `.strict()`, so a hostile save cannot park arbitrary keys in the row. Note the two flag lists in that module—`preferTwisty` (#28) belongs in normalizing, hashing and the cache key and must never reach `routeModifiers`, which Routes would reject.
