...
- **The build's version is computed from the commit, and the deploy prints it.** `APP_VERSION` is `YYYY.MM.DD.N`; the banner shows it next to the git SHA before it asks for confirmation. Use that string as the heading when you add the matching entry to `src/content/release-notes.html`, so a rider reading the footer can find it.
