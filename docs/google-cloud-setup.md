...
- App logo—the stacked dark artwork at 1840×1036; Google renders it on a white consent screen. It is a one-off console upload and is **not in the repo**: it lives in Ziad's local `_assets/`, which is not committed. Anyone else doing this setup needs the file from him, or can re-export it from `public/img/logo-routeloop-dk.svg`.
