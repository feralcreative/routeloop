...
| Route                                                            | Notes                                                                                                                                                                                       |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /`                                                          | The dashboard, signed-in (`routes/home.tsx`); the public ride listing is `/explore`. Listed here by mistake since the row was written                                                    |
| `GET /m/:slug`                                                   | Viewer page—one shell for both sources                                                                                                                                                      |
| `GET /m/:slug/go`                                                | The on-the-road page (#69): the Google Maps hand-off—each route as ordered `/maps/dir/?api=1` links carrying 9 waypoints plus two ends, every one starting from the rider, all three densities in the page—plus send-to-device, the roadbook as a list, and Keep on this phone. `routes/go.tsx` |
| `GET /m/:slug/navigate`                                          | 301 to `/m/:slug/go`, query carried. The page's address until 2026-09-14. |
| `GET /m/:slug/keep.json`                                         | The kept copy's manifest—slug, title, updatedAt, the go page and roadbook URLs, and the files (GPX only since 2026-09-17)—for a Keep sign pressed somewhere other than the go page. Same gate as the page (`viewableRide`), `no-store`. `routes/go.tsx` |
...
| `GET /@username`                                                 | Public profile (`/:handle{@…}` in `routes/pages.tsx`). How much a viewer gets is `profileDepth()` in `src/profiles/policy.ts`: the full page, or the name, handle and friend and follow buttons when the rider's visibility shuts the viewer out (#395) |
...
`POST /api/rides/:id/rendezvous` proposes meeting points for one subgroup, owner-only. The whole computation is pure geometry and calls no router—ranking candidates through the Routes API would be a Routes bill per keystroke, and the proposal is a suggestion the planner accepts, at which point the ordinary routing path draws the real road. An empty `candidates` with a `reason` is a real answer: `no-trunk`, `no-days`, or `none-viable`. The body may carry `maxDivertMi`, how much farther out of their way than their cheapest possible meet a joining group may be sent (#370); an unusable value falls back to the rider's `user_profiles.meet_divert_mi`, then to the app's default of 10. Each candidate's `diverts[].mi` is that group's road extra over their cheapest way of joining where the road was measured, straight-line where it was not. A joining group with a starting point and no route of its own costs one cached Routes request per press for its direct road, which is what places the convergence.
...
**A route carries a SET of groups.** The GET's `routes[].groups` is derived from who is on the route by their HOME group (`ride_members.subgroup_id`), not from what the route stores—once a group merges its riders' stored group is null, so a shared route reports the groups that actually contribute rather than claiming everybody. `route_riders.subgroup_id` is who a rider is RIDING AS there: null is the main group, and a named group survives on the feeder route it rode, which is what a later split reads back. The PUT takes `riders: [{id, group}]`; a bare `riderIds` array is still accepted and read as everyone riding as the main group. Both answer with the same shape. `routes.subgroup_id` is derived alongside for one release—one group where a route carries exactly one, null otherwise—so `strandOf`, the proposer and the viewer's dimming need no change yet.
...
| `POST /api/places/autocomplete`                               | `{q, session}` in, `{suggestions: [{id, main, secondary}]}` out. Places Autocomplete for the profile's "Look it up" box (#396). `session` is a token the client mints per lookup, so the keystrokes and the one Details call bill as a single session |
| `POST /api/places/details`                                    | `{id, session}` in, `{name, label, lng, lat, parts}` out: the picked place's coordinates and its address split into the four profile fields. `name` is empty unless the place is a named business or landmark rather than an address |

**The rider's avoid list is applied to `/api/places/search` AFTER the cache** (#271), never inside `searchPlaces`: that cache is keyed on the query, so ranking inside it would either fragment it per rider—paying Google again for a search somebody else already made—or serve a list pre-ranked for whoever asked first. It is a **demotion and never a removal**, so the response still carries every result Google returned. It reaches `POST /api/rides/:id/rendezvous` as a nudge of a few miles of score on a named station (#370, reversing the 2026-09-07 rule), never as a removal.
...
| `GET /`                            | The dashboard—hero miles, tiles, storage meter, records, twelve-month chart from `src/stats/`, and a count with one link to `/rides`. `routes/home.tsx`. The ride lists lived here from 2026-08-24 to 2026-09-15 |. **A phone is sent to `/rides` with a 302** since 2026-09-17 (`isPhone()` in `src/device.ts`, the client hint then the User-Agent) |
| `GET /dash`                        | The dashboard with no redirect, on every device; what the Dash menu item links to since 2026-09-17. `routes/home.tsx` |
| `GET /rides`                       | The ride lists, `requireActive`: Your rides (owned, then the rides somebody else put you on), Friends, Following, Public, Recycle bin. `?tab=` opens a tab server-side (`/trash` lands on `?tab=bin`); `?show=all` lifts the cap on the owned list to a higher ceiling rather than removing it. `routes/rides.tsx`; the manifest's `start_url`. Was a 302 to `/` from 2026-08-24 |
| `GET /rides/keep.json`             | `requireActiveApi`. Every ride the On this phone switch may keep—owned to `RIDE_CEILING` plus the rides somebody else put you on—as `{slug, title, updatedAt}`; rides.js walks this rather than the capped page. `no-store`. `routes/rides.tsx` |
| `GET /dashboard`                   | **301 to `/`**—301 to `/rides` from 2026-08-15, repointed at the destination on 2026-08-24 so it does not chain through a second redirect. Left pointing at `/` on 2026-09-15: Dash is `/`, and the 301 is cached in every browser that saw it |
| `GET`/`POST /profile`              | The **Profile tab of `/settings`** since 2026-09-07 (#269), not a page of its own. Kept rather than redirected, the way `/friends` opens the Friends tab of `/riders`—it is linked from the account menu and from bookmarks. Owns the profile form's validation and its 400 re-render, which comes back as the whole two-tab page with the Profile tab open |
...
| `POST /settings/tips`              | Still accepted and still writes the column, but nothing reads it and no form posts here since #397: tips are on for everyone |
| `POST /settings/notifications`    | One group of the Notifications topic: its email, browser and mute switches for every kind in the group (the hidden `group` field says which, so an unticked box reads as off). Muting a kind here also marks anything of that kind still unread as read (#402) |
| `GET /notifications`              | The notification center. Reads the list, then marks it all read. A muted kind's rows say Muted and carry an Unmute sign; the rest carry STFU |
| `POST /notifications/mute`        | `{event, muted}` from a row's STFU or Unmute sign, `requireSameOrigin`, redirects back to `/notifications`. An unknown event writes nothing (#402) |
| `POST /api/notifications/pending` | The poll. Claims and returns this rider's undelivered rows for the kinds they want in the browser and have not muted, stamping `delivered_at` so two tabs cannot raise one twice. A POST because it claims |
| `POST /api/tour/start`             | Starts the guided tour: bins the rider's previous tour ride, creates a fresh one from the seed in `src/tour/seed.ts`, puts the three guide riders on it (creating them the first time a deployment sees a tour—see `src/tour/guides.ts`), and stamps `user_profiles.tour_ride_id`. `requireActiveApi` + `requireSameOrigin`, JSON `{id, slug, guides}`, 201; 503 if the guides cannot be created |
| `POST /api/tour/done`              | The guided tour was finished or skipped (#133). Takes `{rideId}`, bins the ride when it is the one `tour_ride_id` names, stamps `tour_done_at` and clears `tour_ride_id`. One endpoint for both outcomes, because a rider who pressed Skip has been offered it and must not be offered it again on every load. Idempotent. `requireActiveApi` + `requireSameOrigin`, JSON `{ok:true}` |
