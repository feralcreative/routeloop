...
| `POST /settings/tips`              | Show me around, on or off (#133). One preference, one column, same INSERT seeding as the row above. The default is `on` and a value it does not recognize lands there, so the only way to turn this off is to say `off` |
