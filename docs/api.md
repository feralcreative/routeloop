...
`POST /api/rides/:id/rendezvous` proposes meeting points for one subgroup, owner-only. The whole computation is pure geometry and calls no router—ranking candidates through the Routes API would be a Routes bill per keystroke, and the proposal is a suggestion the planner accepts, at which point the ordinary routing path draws the real road. An empty `candidates` with a `reason` is a real answer: `no-trunk`, `no-days`, or `none-viable`. The body may carry `maxDivertMi`, how much farther out of their way than their cheapest possible meet a joining group may be sent (#370); an unusable value falls back to the rider's `user_profiles.meet_divert_mi`, then to the app's default of 10.
