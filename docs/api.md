...
| `POST /api/tour/done`              | The guided tour was finished or skipped (#133). Stamps `user_profiles.tour_done_at`; one endpoint for both outcomes, because a rider who pressed Skip has been offered it and must not be offered it again on every load. `requireActiveApi` + `requireSameOrigin`, JSON `{ok:true}`. Nothing un-does it: re-running the tour is a client action |
