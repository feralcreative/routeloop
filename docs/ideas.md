...
- Multi-day ride planning
  - A "Ride" is a package containing many routes over many sessions/days
- Routes have a starting point and and ending point, a starting day/time and an ending day/time, which comprise the length of the Ride.
- Across the bottom of every Ride and Route is a slider, from the start time to the end time that user can slide to see different parts of the route at different times. The overall ride/route is always visible, but the leg/section that corresponds to that general date/time.
- Users should be able to plan routes, label stops/waypoints, add umpteen routes to a given ride.
...
- No platforms, no limits.
  - Google maps offers a max of 10 waypoints. Apple maps yields different functionailty depending on the device. The point of this app is not realtime navigation and never will be—it's a planning and sharing tool. An app to finally allow riders to see a holistic view of their ENTIRE ride, every leg, every session, every stop, every hotel, every gas station, across an unlimited number of days and miles.
