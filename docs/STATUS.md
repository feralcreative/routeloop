...
**Branch:** `feat/public-profile` ([#395](https://github.com/feralcreative/routeloop/issues/395)–[#400](https://github.com/feralcreative/routeloop/issues/400)), stacked on `chore/condense-comments` and shipping as one PR with it—the public rider profile with a visibility setting, a Google Places lookup for Home base and Public starting point, the Show me around card removed with tips and the tour sign on for everyone, folder-tab card subheads, compact rider-card buttons, and the codebase's comments cut by roughly half. **Migration `drizzle/0045`, additive (`profile_visibility` enum, `bio`, `share_paddock` on `user_profiles`), so prod deploys first.** Cut from `main` at `9a78a26`. **3,136 tests across 126 files** (3 skipped, 3,139 total).
