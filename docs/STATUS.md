...
Ziad's calls. `/` sends a phone to `/rides` (302, `isPhone()` in `src/device.ts` from the client hint and the User-Agent, tablets read as desktops) and `/dash` is the dashboard with no redirect, which is what the Dash item links to now; `dash` is a reserved handle. On a phone the ride cards are rows—the thumbnail at 25vw on the left, and to its right the title, a big green Load ride sign with a small blue Edit ride beside it (Riders, brown, on a ride somebody else planned), then the pill and Delete—because the number one use of a phone is loading a planned ride into a GPS and Edit must not compete; `.ride-card-link` takes `display: contents` so the face and body join the card's grid. The signs render on every card and the phone rule alone shows them. Verified in emulated Chrome at 390×844 with an iPhone UA: `/` lands on `/rides`, a row is 164px tall with the picture covering it at 98×164, Load 160×49 and Edit 73×29, no horizontal overflow; at 1280px the block is `display: none` and `/` is the dashboard. **3,070 tests across 120 files.**
