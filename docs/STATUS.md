...
**What is on main since the last deploy:** a brighter dark mode—the dark ramp's text half lifted two steps, black ink on the Changed and Removed badges, the map-page tour sign's leftover green ring, doubled route-grip tints—plus `user_profiles.map_scheme` (`drizzle/0043`, additive) with a Map tiles control on Appearance, `/signs.html` for checking every sign in every palette, and a dev light/dark flip. **The last of those is a migration, so prod deploys first.**
