...
**Branch:** `fix/ui-polish` ([#405](https://github.com/feralcreative/routeloop/issues/405)–[#408](https://github.com/feralcreative/routeloop/issues/408)), cut from `main` after #404 merged—the colorblind palette's sign fields retuned by measurement with a test holding them apart, the overrides table's Custom pill matching the Default one and its examples reading with the word in force, a tick-all box per notification column, the power choices renamed, and the dev Light/Dark flip no longer pinning light. No migration. **3,150 tests across 126 files** (3 skipped, 3,153 total).
**The branch before it, merged 2026-09-24 as [#404](https://github.com/feralcreative/routeloop/pull/404):** `feat/mute-notifications` ([#402](https://github.com/feralcreative/routeloop/issues/402), [#403](https://github.com/feralcreative/routeloop/issues/403))—mute any notification kind from Preferences or an STFU sign in the center; muted rows land already read, with no badge, popup or email. Short names for every kind. No migration.
**The branch before it, merged 2026-09-24 as [#401](https://github.com/feralcreative/routeloop/pull/401):** `feat/public-profile` (#395–#400), shipped with `chore/condense-comments`—public rider profiles, the Places lookup, folder-tab cards, compact rider-card buttons, and the comment trim. **Migration `drizzle/0045`, additive, so prod deploys first.**
**The branches before it, merged 2026-09-22 and 2026-09-23 as [#392](https://github.com/feralcreative/routeloop/pull/392) and [#394](https://github.com/feralcreative/routeloop/pull/394):** `feat/preferences-pass` ([#389](https://github.com/feralcreative/routeloop/issues/389)–[#391](https://github.com/feralcreative/routeloop/issues/391))—every choice card's sentence moved into a tooltip and a per-group `?` key, Appearance four across and Units five, the power picker following the vehicle, and the vocabulary overrides as one four-column table—then `fix/update-email-notifications` ([#393](https://github.com/feralcreative/routeloop/issues/393)), the beta email lockup at half the height. No migration.
**The branch before it, merged 2026-09-21 as [#387](https://github.com/feralcreative/routeloop/pull/387):** `feat/beta` ([#376](https://github.com/feralcreative/routeloop/issues/376)–[#386](https://github.com/feralcreative/routeloop/issues/386))—the beta sprint: one release-stage switch and the site says closed beta, a sneak peek of the tour on the sign-in page recorded from a seeded database, the splash as a folder of shuffled crossfading clips with a pipeline to cut them, dark mode on the splash as a scrim, the beta sign superimposed on every wordmark and baked into the email and OG marks, the favicon set regenerated, dark cards lifted again, and twistiness drawn as a line that gets wavier. No migration. Cut from `main` on 2026-09-20 after #375 merged. **3,126 tests across 125 files** (3 skipped, 3,129 total).
**The branch before it, merged 2026-09-20 as [#375](https://github.com/feralcreative/routeloop/pull/375):** `fix/bin-purge-digest` ([#374](https://github.com/feralcreative/routeloop/issues/374))—the purge warning is one notification per rider about their whole bin, bumped rather than repeated: `bin-digest.ts` for the copy, `notifyOnce()` for the bump, the email listing the bin. No migration. **3,100 tests across 123 files** (3 skipped, 3,103 total).
**The branch before it, merged 2026-09-20 as [#373](https://github.com/feralcreative/routeloop/pull/373):** `fix/meeting-point-ranking` ([#370](https://github.com/feralcreative/routeloop/issues/370), plus the retroactive [#371](https://github.com/feralcreative/routeloop/issues/371) and [#372](https://github.com/feralcreative/routeloop/issues/372))—the meeting-point proposer scores a trade instead of taking the earliest point under the cap, the allowance is measured from each group's cheapest meet rather than from zero, the default drops from 25 to 10, and a rider sets their own on `/settings`. **Migration `drizzle/0044`, additive (`user_profiles.meet_divert_mi`), so prod deploys first.** **3,092 tests across 122 files** (2 skipped, 3,094 total).
**The branch before it, merged 2026-09-17 as [#369](https://github.com/feralcreative/routeloop/pull/369):** `fix/dark-mode-contrast` ([#368](https://github.com/feralcreative/routeloop/issues/368))—the dark scheme read back on a live dashboard: the page below the cards for the first time (`$page`, and `$white` lifted to `#1a1a1a`), the muted grays up a pair with `$concrete` cut loose from the ramp, a hairline on cards, two dashboard slots retoned, and the tour's spotlight ringed in `$yield` so it exists in the dark at all. No migration. **3,080 tests across 122 files.**
**The branch before it, merged 2026-09-17 as [#367](https://github.com/feralcreative/routeloop/pull/367):** `feat/on-the-road` ([#69](https://github.com/feralcreative/routeloop/issues/69), plus the retroactive [#354](https://github.com/feralcreative/routeloop/issues/354), [#355](https://github.com/feralcreative/routeloop/issues/355), [#356](https://github.com/feralcreative/routeloop/issues/356), [#359](https://github.com/feralcreative/routeloop/issues/359), [#360](https://github.com/feralcreative/routeloop/issues/360), [#361](https://github.com/feralcreative/routeloop/issues/361)); plus [#365](https://github.com/feralcreative/routeloop/issues/365) and [#366](https://github.com/feralcreative/routeloop/issues/366))—the on-the-road page, send-to-device and the service worker (2026-09-14), the ride lists moved off the dashboard to `/rides` and the phone pass (2026-09-15), then a phone opening on the list with a Keep on every card, the kept copy cut to GPX, and the On this phone bulk switch (2026-09-17). **3,075 tests across 121 files** (2 skipped, 3,077 total). No migration. Rebased onto `main` after #364 merged. **Not yet driven on a phone**—see the checklist under On the road; every phone pass here was emulated Chrome at 390×844. **The phone list ships clunky on purpose**: Ziad's call, 2026-09-17, to merge and come back to the styling.
**The branch before it, merged 2026-09-17 as [#364](https://github.com/feralcreative/routeloop/pull/364):** `feat/faq-privacy-analytics` ([#362](https://github.com/feralcreative/routeloop/issues/362), [#363](https://github.com/feralcreative/routeloop/issues/363))—usage analytics (Google Analytics 4 plus the Cloudflare Web Analytics beacon, prod only, with a consent bar in the EU and nowhere else), the privacy page rewritten to say so, and three FAQ entries under Your data. Cut from `main` on 2026-09-17 while `feat/on-the-road` (#69) waits unmerged; that branch rebases onto this once it lands. No migration. Two optional env keys, `GA_MEASUREMENT_ID` and `CF_BEACON_TOKEN`, which need `push-env` before the deploy that should count. **3,015 tests across 115 files** (2 skipped, 3,017 total).
**Before that, merged 2026-09-15 as [#358](https://github.com/feralcreative/routeloop/pull/358):** `fix/replace-point-search` ([#357](https://github.com/feralcreative/routeloop/issues/357))—a point's name field in the builder is the place search, and a pick replaces the point where it sits. Cut from `main` while this branch waited; this branch was rebased onto it on 2026-09-15. No migration.
**Before that, merged 2026-09-15 as [#353](https://github.com/feralcreative/routeloop/pull/353):** `fix/contrast-in-dark-mode` ([#349](https://github.com/feralcreative/routeloop/issues/349), [#350](https://github.com/feralcreative/routeloop/issues/350), [#351](https://github.com/feralcreative/routeloop/issues/351), [#352](https://github.com/feralcreative/routeloop/issues/352))—the dark-mode pass, the map-tiles preference and the Your stats reshape of 2026-09-14, committed as they landed. `main` is `f1f2227`. Before it `chore/minify-dist` ([#348](https://github.com/feralcreative/routeloop/pull/348)) merged, after `feat/dashboard-polish` ([#346](https://github.com/feralcreative/routeloop/pull/346)), `feat/account-polish` ([#344](https://github.com/feralcreative/routeloop/pull/344)) and `feat/chrome-polish` ([#345](https://github.com/feralcreative/routeloop/pull/345)). **2,986 tests across 114 files** (2 skipped, 2,988 total).
...
**What this branch is:** UI fixes, #405–#408. The colorblind palette's nine sign fields in `_palette.scss` were searched in OKLCh for the widest smallest separation under normal, protan, deutan and tritan vision within each field's hue family and legend ink; every pair is now at least 16 apart (about 12 tritan), and `test/viz-categories.test.ts` holds floors of 15 and 10. The overrides examples are templates filled by `exampleParts()` and refilled live by `jargon.js`. `public/js/notif-columns.js` is the tick-all. The dev flip stores only a press under a renamed key. Verified in Chrome: the palette strip in colorblind dark, a live example with a typed word (then restored), a column flip saved off and back on, and a planted stale "light" dropped on reload.

**The branch before it was** muting notifications, #402 and #403. `MUTE`, `isMuted()` and `delivers()` in `src/notifications/policy.ts`; every send path asks `delivers()`, and a muted kind's row is written already read and delivered (`quietStamps()`), so the badge's query is untouched. Stored as `channel = 'mute'` in `notification_prefs`. The Mute column on Preferences and the STFU/Unmute sign on center rows (`POST /notifications/mute`) both quiet anything still unread of that kind. `data-tip-strong` on the tips layer bolds the kind in the tooltip. Verified in Chrome: muting from the center and from Preferences, the Muted row state, the fast tooltip, and a real `notify()` of a muted and an unmuted kind against dev (read and delivered versus unread; test rows deleted).

**The branch before it was** the public profile sprint. `src/profiles/policy.ts` decides how much of `/@handle` a viewer gets (`profileDepth()`: owner and `public` full, `members` full for an active signed-in rider, everyone else the name, handle and friend and follow buttons) and `service.ts` loads it; stats count listed rides only, and `canSeePaddock()` also gates `/bikes/:id/photo`, which was owner-only. The Profile tab's "Your public page" card holds the three-level setting, the bio and the Paddock switch. Home base and Public starting point have a "Look it up" box on Places Autocomplete and Details, proxied in `src/maps/places.ts` with one session token per lookup, replacing the Geocoding dropdown on the address line. The Show me around card is gone: nothing stamps `data-tips` and `hide_tour` is unread. Card legends on `.profile-form > fieldset` are folder tabs with a real gutter between cards, and rider-card verbs on `/riders` are one compact, centered, side-by-side size. The comment pass cut 61 files by a net ~5,300 lines with no behavior change, and carried a Preferences copy pass (Motion opens "Match my device", "Map theme", today's date three ways, "What makes you go?"). Verified in Chrome at 1440 and 390 wide: the profile at all three levels signed in and out, the bike photo gate, the autosave of the new fields, the folder tabs with their save outline, and the riders page at all three card sizes. The live Places lookup was tested by Ziad, not by the agent.

**The branch before it was** the Preferences pass, #389–#391. `Choice` in `views/account-page.tsx` renders every radio card with its sentence as `title` and as tips.js's new `data-tip-inline` body (no headline, the title is the body), kept inside the label as hidden text; `choiceKey()` puts a `?` beside each group's heading listing every option with its sentence, built from the same lists; `.three-col--four`/`--five` hold Appearance and Units on one row from 992/1200px. The power picker grays the powers the vehicle cannot use (`toPower()`'s rule, server-rendered and re-gated by `jargon.js`), one direction only. The overrides table is Term, Example, Default, Custom at 25% each with `table-layout: fixed`: a default pill carrying the pickers' word (the head names the pair, "Default (Motorcycle, Gas)") and a 24ch custom box, the handler unchanged in what it stores, and a fuel row hidden under Pedal keeping its inputs so a save no longer drops the rider's fuel words. Verified in emulated Chrome at 1440, 1200, 1000, and 992 wide.

**The branch before it was** the beta sprint, #376–#384. `src/views/stage.ts` is the switch (`closed-beta`; `public-beta` is one edit) and every stage phrase on the site reads it, with `BETA_SIGN` derived from it. The sneak peek: `utils/record-tour-replay.ts` drives the real tour headless against the seeded `routeloop_tour` database (Demo Rider `@demorider`; re-record with a dev server on `DATABASE_URL=…/routeloop_tour DEV_LOGIN_EMAIL=demo@routeloop.app npm run dev:server`) into `public/tour/replay.json` and 43 WebP frames; `public/js/replay.js` plays it in a native dialog with a fixed button bar, opened by a yellow Sneak peek sign under the sign-in block; two tour bugs the recording found are fixed (`TBBuilder.ready` awaited, `reanchorOn()` watching `#info-panel`), and the split pair's adrift target is #385. The splash: 31 clips in `public/video/splash/`, `site.js` shuffling and crossfading two `<video>` layers with a six-clip cap, `utils/splash-clips.sh` as the three-stage pipeline (`--scan`, run, `--only`, `--sheets`, `--prune`), and dark mode there a `$splash-ink` scrim with every foreground token pinned light. The beta sign: `beta-lockup.svg` overlaid by `wordmark()` in percentages of the mark (Ziad caught the pixel nudges drifting), `utils/build-email-logos.mjs --beta` for the email PNGs, the OG card composited, the favicon set regenerated from `_assets/favicon/` with the 192/512 in the maskable slots too. Dark cards: `#242424`, the border ramp up a pair, `$card-edge` 12%, the dark link on an absolute lightness. Twistiness: a rank per band, `twistScale()` on both sides drawing the band's line—the stylesheet turns an empty `<i data-rank>` into a line that gets wavier—beside the band's word, with `twistKey()` in a `?` bubble on the hero showing all five (five curve marks, then the line alone, were built and rejected), and the hero saying "Roughly N hours of riding" with no asterisk. Verified in emulated Chrome at 1440 and 390 wide: the sneak peek, the scrim, the clip cycle and its cap, the sign on every wordmark, the dark dashboard and ride list, and the twist line on the hero, the record, the viewer legend and the builder totals.
**The branch before it was** #374—one notification for the bin. `warnRidePurges` still stamps each ride the hour it comes within a week of going, then rebuilds the owner's one `trash_purge_soon` row from everything in their bin; `notifyOnce()` rewrites it in place, resets its clock, clears read and delivered, and deletes any other rows for the event—which is what sweeps the old pile. `collapseBinWarnings()` on the same sweep folds the existing pile silently—newest row rewritten, rest deleted, read state kept—so prod's pile goes on the first sweep after the deploy rather than on the next bump. Verified against a prod copy pulled into dev: the real sweep wrote one row listing thirteen rides, and a seeded pile of four folded to one with a second pass a no-op. One digest email was sent to Ziad from dev by that run.
**The branch before it was** the fix for #370—`DIVERT_WEIGHT` and a two-pass `proposeGroupMeet` in `src/subgroups/rendezvous.ts`, `withDefaults()` closing a cap that an explicit `undefined` switched off, a Meeting points topic on Preferences posting to `/settings/meet`, the builder's dial seeded from the preference and relabeled "extra detour", and the route reading press, then preference, then default. Verified in the browser: the preference saves, clears to null and clamps 900 to 200, the dial opens on the stored number, and Find meeting points on ride 112 (epim's repro, built by Ziad) answers with the Livermore junction first.
**The branch before it was** a brighter dark mode—the dark ramp's text half lifted two steps, black ink on the Changed and Removed badges, the map-page tour sign's leftover green ring, doubled route-grip tints—plus `user_profiles.map_scheme` (`drizzle/0043`, additive) with a Map tiles control on Appearance, `/signs.html` for checking every sign in every palette, a dev light/dark flip, and the dashboard's stat tiles as three bars each (you, the average rider, the top rider) under a Your stats heading with the Plan a ride sign beside the hero. **The map-tiles column is a migration, so prod deploys first.**
**The branch before that:** the production image minifies `public/js` and strips the comments out of `src/content`—`utils/minify-dist.mjs`, esbuild, one `RUN` in the Dockerfile—so the 1.24 MB the server was sending verbatim is 429 KB. Dev tree untouched; `--in-place` refuses inside a git checkout. No migration. Its first run found a dead duplicate key in `MEET_REASONS` left by the day→route rename.
...
**PROD NEEDS THE MIGRATION FIRST**: this branch carries `drizzle/0045`; `utils/deploy/both.sh`, prod first.
**For:** the next agent, or the owner returning cold

## Public profiles, and who can see them, 2026-09-24

`feat/public-profile`, #395–#400, shipping with `chore/condense-comments` in one PR. `/@handle` is a real profile: picture, bio, joined month, friend and follower counts, stats from listed rides, the Paddock if shared, and the public rides, behind a three-level visibility setting (Anyone, Riders, Nobody) that defaults to Riders. Home base and Public starting point have a Places lookup. The Show me around card is gone and tips are on for everyone. Card subheads are folder tabs, and rider-card buttons line up. Migration `drizzle/0045`, additive; prod deploys first.

## Split tour cards keep their targets, 2026-09-21

#385, on `feat/beta`. New routes inherit a folded state when the preceding list had no open section at their position; the meeting overview restores only the routes it folded itself. The split cards now open the route containing their target whenever Shepherd resolves it, including after a frame replaces the row and when Back restores the earlier scene. The Pescadero row and its departure line stay visible without opening unrelated routes. Six regression cases cover folded, already-open, replaced, and missing targets. The two preview frames were re-recorded on the isolated, seeded local `routeloop_tour` database, and the recording test no longer exempts either card from having a visible target. Verified in Chrome: the live split card highlights Pescadero and the following card highlights the departure line. Typecheck, 3,126 tests across 125 files (3 skipped), and all three prose checks pass on Node 24. No SCSS change.

## One warning for the bin, 2026-09-20

`fix/bin-purge-digest`, #374. Rides started reaching the end of their thirty days in batches and the per-ride purge warning became a pile. It is one notification per rider now, listing every ride in the bin with its date, soonest first, cut to the 400-character column by dropping names rather than truncating one (`src/notifications/bin-digest.ts`, nine tests). `notifyOnce()` in `service.ts` is the bump—rewrite the existing row, reset `created_at`, clear `read_at` and `delivered_at`, delete any others for the event—so one row is an invariant and a rider's old pile is swept the first time they are bumped. The per-ride `purge_warned_at` stamp still decides when. The email lists the bin the same way; the template's old defense of per-ride messages is struck in its header.

## The meeting point is a trade, 2026-09-19

`fix/meeting-point-ranking`, #370. epim's screenshot: Scotts Valley to Twain Harte with a second group from San Francisco, every proposal sending the SF group twenty-five miles out of their way. The 2026-09-03 rule—earliest point under the cap, divert at a tenth—always landed at the cap, and on a group that starts near the road that is a group sent backwards. The score is `alongM + 1.5 × divert` now: a mile out of the way has to buy a mile and a half together, which keeps a group at the junction where its own road joins (two miles per mile behind it) and still sends a group far off to one side earlier (about half a mile per mile, ride 34's shape). The cap is measured from each group's cheapest viable meet, in two passes, so a distant group gets its cheapest point instead of "nowhere works"; `fuelOnly` measures the bare road for the floor and never offers it. Default 10, and `user_profiles.meet_divert_mi` (`drizzle/0044`) for a rider's own, nullable with the default in code; the builder's dial seeds from it and says "extra detour". **Then driven on epim's ride and found still wrong**: the straight line to Fremont crosses the bay (9.6 paper miles, 21 by road) and a start-only group had no track for the on-route test. A start-only joining group now gets its direct road fetched once as its track (one cached Routes request per press), `rankByRoad()` re-orders the routed shortlist by road miles with the cap on the road extra, `reachable()` routes until the budget is spent rather than until three are kept, the convergence is front-loaded into the search anchors and the routing order, and stations spread at 3 km. Verified on ride 112: Livermore Costco and Shell at zero, Sunol +14.8 and Fremont +21.2 behind them. **Then the Costco was on Ziad's avoid list**, so the two place lists now reach the proposer as a nudge of eight miles of score in both passes—reversing the 2026-09-07 rule, never a removal—and ride 112 answers Chevron, Springtown, Ruby Hills. **Found on the way:** `{ maxDivertMi: undefined }` spread over `DEFAULTS` switched the cap off for a client sending nothing, and its test passed on a fixture the cap never reached. Placeholder columns for future preferences were asked about and declined—an additive migration ships free with its feature, and a guessed column is a rename. See docs/decisions.md.

## Counting visits, and asking first where the law asks, 2026-09-17

`feat/faq-privacy-analytics`. GA4 and the Cloudflare beacon on every page, emitted only when `IS_PROD` (`!IS_DEV && !IS_STAGE`) and a key is set; both keys validated at boot so they can be interpolated unescaped. The visitor's country reaches `page()` through Hono's `contextStorage()`—the one place a view reads the request—and in the EU, EEA, UK, Switzerland, and over Tor the GA half waits behind a blue consent bar in the staging banner's slot. The choice is `localStorage` only; `consent.js` is the one loader in both modes so a stored no is honored anywhere. Verified in the browser with `ANALYTICS_DEV_COUNTRY=DE`: the bar pushes the header and the map by its measured height, Fine by me loads gtag with `npa=1`, No thanks loads nothing and expires the two cookies, both survive a reload, and the switch on `/privacy` works in both modes—a re-grant after a deny on the same page was found stuck behind the once-only guard and fixed. The invite page opts out because its path is the token. Privacy page: five services (Gmail corrected to Resend), the two GA cookies, what GA records and what it is switched off from, the EU prompt, and that GA records the query string where Cloudflare does not. **Before the deploy:** create the GA4 property (Signals off, 14-month retention, Enhanced Measurement on), switch the Cloudflare Web Analytics site to JS snippet and turn auto-injection off, put both values in `.env`, `push-env`.

## Dark mode, read back again, 2026-09-17

`fix/dark-mode-contrast`. Ziad's calls, with every pair passing the audit: cards sat darker than the page, the muted text was murk, and the tour's spotlight—a black overlay at 62% over a near-black page—was invisible. `$page` is the page (-96 light, `#0a0e11` dark) and dark `$white` is `#1a1a1a`, a card one step above it; dark -96 became `#121212`, a well; `$card-edge` is a faint light hairline on cards in the dark. -29 through -70 lifted a pair each and `$concrete` is a fixed `#888888` everywhere so the roster mark's glyph no longer pins -57. `THEME_COLOR` follows `$page`. The contrast theme's two dark sign fields fell within ΔE 18 of the lifted card as chart bars, so the dark dashboard paints `$brand` and `$interstate-text` for those slots, measured by the viz test under both names. The tour overlay's path takes a `$yield` stroke, ringing the cutout; `clip-path: inset(2px)` trims the viewport-edge half of it. Route lines and pins on dark tiles are the stored color lifted to L 58 (`route-ink.js`, `inkFor()` in map-common.js), decided once at map creation from the tiles' own scheme. Checked in emulated Chrome at 1280px under the dev flip: dashboard, `/rides`, the viewer on 2021 Colorado, and the tour's first spotlight.

## A phone opens on the list, two across, 2026-09-17

Ziad's calls. `/` sends a phone to `/rides` (302, `isPhone()` in `src/device.ts` from the client hint and the User-Agent, tablets read as desktops) and `/dash` is the dashboard with no redirect, which is what the Dash item links to now; `dash` is a reserved handle. On a phone the ride cards are rows—the thumbnail at 25vw on the left, and to its right the title, a big green Load ride sign with a small blue Keep beside it, then the pill, a small amber Edit ride (Riders, brown, on a ride somebody else planned) and Delete—because the number one use of a phone is loading a planned ride into a GPS and Edit must not compete; `.ride-card-link` takes `display: contents` so the face and body join the card's grid. The signs render on every card and the phone rule alone shows them. Verified in emulated Chrome at 390×844 with an iPhone UA: `/` lands on `/rides`, a row is 164px tall with the picture covering it at 98×164, Load 160×49 and Keep 73×29, no horizontal overflow. **Keep on the card** goes through `public/js/keep.js`, extracted from go.js, and `GET /m/:slug/keep.json` for the manifest; pressed on the list it stored 328 kB (page, roadbook, files, registry row) and the go page then read the same copy back as Kept, removed it, and kept it again. **The kept copy and the On the road page are GPX only now**—KML and the Routeloop file were the same geometry twice more, 7.8 MB against 4.2 on an imported ride—and **"On this phone" above the list is the bulk switch**: None, Last 30 days, This year, All, by when a ride last changed (`keep-policy.js`, pinned zone-proof), synced from `GET /rides/keep.json` on every online visit, removing only what the policy itself kept (`via` on the registry row). Measured: This year kept 14 rides, 21.5 MB, in about a second on the LAN; None took the twelve it had added back out and left the two hand-kept ones; at 1280px the block is `display: none` and `/` is the dashboard. **3,080 tests across 122 files.**

## The ride lists at `/rides`, and the phone pass, 2026-09-15

#359 and #360, Ziad's calls. The five ride lists and "Riding with others" left the dashboard for `src/routes/rides.tsx`; `/` keeps the numbers and closes with a count and one link. The joined rides sit INSIDE Your rides after the owned ones, each card saying who planned it (`ridesImOn` returns `owner`). `?show=all` replaced `?rides=all`; `?tab=bin` still opens the bin and `/trash` lands there through `RIDES_BIN`. `src/rides/tabs.ts` is the pure half with `test/ride-tabs.test.ts`; `src/views/ride-lists.tsx` holds the two cards; `public/js/rides.js` carries the in-place binning unconditionally with the strip's counts moving. Nav: Dash first, Your rides first in the Rides group, `'rides'` back in `NavKey`. `rides` is a reserved handle. The manifest's `start_url` is `/rides`; the worker passes it through, stores nothing, and answers `/offline` without a network (pinned).

**The phone pass is responsive at the same URLs and the builder is untouched.** `$bp-phone` (767px) is for new rules only. Chrome pages take 1rem gutters and the safe-area insets under it—`.page-wrap` and `.site-header` together—and `viewport-fit=cover` is on every page. The header's three items fit 360px: the wordmark drops to 22px and the tour sign reads "Tour" with its full accessible name. `.page-tabs` scrolls sideways with 44px tabs and `tabs.js` scrolls the active one into view. Ride cards go one column with 44px foot links. Viewer: 44px checkbox labels, collapse toggle and download chips; hover guarded on `pointerType` and a tap pins a row; `100dvh` map, `svh` sheet, insets on the collapsed head, the sheet foot and the timeline; **`fitPadding()` measures what the sheet and the timeline cover**, because the even fit put the whole ride under the sheet—2021 Colorado opened on Alberta. The roadbook table scrolls sideways (`.rb-scroll`) and never stacks. **The QR card is a native popover** (#361): the `<details>` card was fixed inside the drawer's stacking context and sat under the timeline bar on every dated ride; the top layer fixes it and `::backdrop` is the scrim.

**Measured in emulated Chrome at 390×844**: no horizontal overflow on `/rides`, `/`, the viewer or the roadbook; every tab 44px tall; Edit and Delete 44px; the legend's labels 44px with a 22px box; the collapse toggle 44×44; the chips 44px. Delete → in-place Undo with the counts moving (13→12, bin 74→75, and back). **Known gap:** `requireActive` sends a signed-out visitor to `/login` with no `next`, so an installed app opened signed out lands on `/` after sign-in. **A judgment left for Ziad:** one column at 390px makes each card the width of the screen and thirteen rides a long scroll; two columns is one number in `_rides.scss` if the faces read too big.

## On the road, 2026-09-14

`/m/:slug/go` replaces `/m/:slug/navigate` (301, query carried). `src/routes/go.tsx` renders all three densities and go.js shows one; every Maps link drops its origin (`fromCurrentLocation: 'every'` in `gmaps-links.ts`)—the old page passed nothing and its copy was false. Progress is `localStorage['routeloop.go.<slug>']`, one place per density keyed on `{routeUid, part}`; `public/js/go-progress.js` is the pure arithmetic, `test/go-progress.test.ts` pins it. GPX, KML and the native JSON are fetched on load and held as `File`s, shared with no await before `navigator.share()` where the browser takes files, downloaded as a blob otherwise; Android is always a download because Chromium's permitted list (`components/browser_ui/webshare/.../ShareServiceImpl.java`, read) has no GPX in it.

**The worker.** `public/js/sw.js`, served at `/sw.js` by a route with `no-cache` (`src/views/sw.ts` fills the build and the hashed precache). `routeloop-shell-<build>` is the app shell, replaced whole at activate; `routeloop-kept` is rider data and is never dropped. A ride URL is served from cache only when `/_kept/<slug>` exists, written by Keep on this phone; everything under `DENY` (the builder's API, notifications, admin, sign-in, every non-public API, and the chrome pages) is never intercepted. `test/sw.test.ts` drives the handlers with a fake `self`. Registration is skipped on a dev host unless `localStorage['routeloop.sw'] === '1'`. The manifest is `minimal-ui` (iOS standalone has no download manager), and every page carries `theme-color` for the two default surfaces, pinned to the palette by `test/theme-color.test.ts`.

**Driven in headless Chrome over CDP** with the session cookie and the SERVICE WORKER TARGET taken offline as well as the page (page-level emulation does not reach the worker's fetches—the first pass looked offline and was not): the kept go page renders styled with its files in hand, the roadbook opens, an unkept ride and an uncached navigation land on `/offline`, which lists the kept ride. Keep reported 5.9 MB for a 13-route ride.

**Hardware pass still to do**, iPhone Safari (tab and Home Screen) and Android Chrome (tab and installed): (1) tap a leg → Maps offers Start from your location; (2) tap, Next, kill the browser, reopen → progress survives, Reset clears; (3) switch density → fresh list, switch back → restored; (4) Send GPX on iOS → note which apps the sheet lists, then Save to Files → Files → open → note which of Rever, Kurviger, Scenic, Calimoto, OsmAnd, Garmin Explore import it and whether stops survive as waypoints; on Android → Downloads → each app's Import; (5) KML and the Routeloop file the same, and `/import` takes the JSON back; (6) Keep, airplane mode, reload → page, legs, files, roadbook; `/` → No signal listing the ride; Remove → No signal; (7) edit on a laptop, reopen online → "changed since" then refreshed; (8) keep on stage, deploy, reload once, go offline → still styled; (9) Android Install button → minimal-ui with a URL bar; iOS Add to Home Screen → whether iOS honors minimal-ui or opens Safari, and whether a blob download shows the download manager; (10) builder autosave, notifications poll, admin and login untouched with the worker active. **One thing outside the repo:** confirm the Cloudflare cache rule for `*.js` honors origin headers for `/sw.js`, or add an exception, or worker updates lag by the edge TTL.

## Your stats, 2026-09-14

#352. Each tile is a three-row bar chart—You in `--viz-cat-3`, Average rider in `--viz-cat-1`, Top rider in `--viz-cat-2`—every bar a share of the top rider's figure, carried as `youPct`/`avgPct` on the `Spread` and pinned in `test/stats-shape.test.ts`. One grid per tile with the rows flattened by `display: contents`, which is what puts the three tracks on one line; three separate grids started them at three x positions on a phone. The eyebrow became a `.stat-block` h2, the label sits beside the figure, the onramp sits at the right of the hero with its bottom on the figure's (measured equal), and the greeting carries half an em under it.

## Dark mode, read back, 2026-09-14

Three passes on a live dashboard with a dev light/dark flip beside the Dev button: text `#bbbbbb` → `#dddddd` → `#eeeeee`, muted `#808080` → `#aaaaaa`, and `$concrete` is the floor—one pair lighter fails the roster mark's white glyph at 3:1. `/signs.html` shows every sign numbered in all six palettes and is how the Changed badge's `$black`-on-orange was named and fixed; the Removed badge moved onto `$concrete` with black ink so the lifted gray could go. The map got its own light/dark (`map_scheme`, `follow` by default) because a dark page must not force dark tiles; google.maps' `colorScheme` is initial-only, so it lands on the next map load. Verified in the browser: light page, dark tiles, from the stored choice.
...
`feat/dashboard-polish`, #342 and #343. The dashboard's tab strip is server-driven now—`?tab=` names any of the five—which is what lets `/trash` land on the bin (the strip and the bin tab moved to `/rides` on 2026-09-15). `src/views/bin.tsx` holds the two fragments (rides for the list page, places and groups for the fold on `/places`) because `account-page.tsx` imports no route module. Each restore form carries a hidden `back`, honored only as a path so a forged value cannot send a rider off-site; checked live with a scheme and a protocol-relative value, both fell back. The stat row's pair reads `everyone` / `most` at its old size and an eyebrow captions the row once.
...
[#133](https://github.com/feralcreative/routeloop/issues/133), on branch `feat/show-me-around`. **Built, driven end to end in the browser, not merged.** The issue asked for richer tooltips and described the effect it wanted as "a guided first pass"; the tooltips were built first and Ziad's verdict on them was "so subtle and sparse that it's nowhere near the UX I want." The tour is the feature. The tips are the reference layer under it.

**The tour is Shepherd.js 15.3.0 from jsdelivr, and how it loads is the thing to know.** Shepherd 12+ is ESM-only and cdnjs hosts none of its JS past v11, so it comes through a `<link rel="modulepreload" integrity>` in the head plus a dynamic `import()` from `public/js/tour.js` after `load`—an import specifier cannot carry an integrity hash and a preload can. **Not an inline `<script type="module">`**, because module scripts are deferred and execute in document order, so one placed above `builder.js` would hold the builder until jsdelivr answered. If the CDN fails the import rejects and the builder is untouched. `style/_tour.scss` re-themes it by specificity, since the vendor sheet lands after `main.min.css`.

**Three parts, any order, any skippable.** The route, the clock, the people—red, yellow, green, drawn as signs on the welcome and closing cards, with a color band on every card. Four steps wait for the rider (name the ride, first point, second point, a start time) and watch the DOM rather than clicks: a `MutationObserver` on `#route-list` plus a predicate over what is on screen. **The name step waits for a commit** (`change`, which Enter produces), not a keystroke—Ziad got as far as "Coast" before it moved on. **A satisfied wait reached by Back shows Next** instead of bouncing forward; direction is read at the tour's own `show` event. Steps on the Groups and Riders tabs open their tab in `beforeShowPromise`.

**Four traps found by driving it, each recorded in `tour.js`:** the builder re-renders the route list on every edit and a card pinned to a detached node slides to (0,0), so `reanchor()` re-shows the step when `getTarget()` is disconnected; `querySelector(".add-search")` answers with a hidden insert-slot copy, so `anchor()` takes the first VISIBLE match; Shepherd's overlay swallows every click outside the cut-out, so the point steps cut a second hole over the map and `html.tour-active` lifts the search dropdown above the backdrop; and **Shepherd binds its keyboard handler to the target as well as the card**, hijacking Tab from the last focusable inside it—on a `datetime-local` that is the year-to-hour move, so date and time inputs swallow Tab first.

**Storage is two columns.** `user_profiles.tips` (`on`/`off`, default on) and `user_profiles.tour_done_at`—the one nullable column in that block, deliberately, because "never offered" is what decides whether the tour runs unasked and a boolean cannot tell it from "skipped at step one." `POST /api/tour/done` stamps it on Finish and Skip alike. `Take the tour` is in the account menu and on Preferences as a real link to `/builder?tour`; `?tour` on any builder URL starts it.

**The hover tips are `public/js/tips.js`, and the table holds only the BODY.** The headline is read off each control's own `title` at hover time, so `off` is literally the native tooltip and the two strings cannot drift. Forty keys across the builder, the viewer, the download row, the dashboard tabs and the footer; `test/tips.test.ts` fails on a key with no copy, copy with no key, and every tour anchor that does not resolve. **Removing `title` can remove an accessible name**, so a control with no other name gets an `aria-label` from it.

**Not done, and the reason:** widening the tips further (the roster, riders, survey pages) was offered and not asked for. The three-wait shape Ziad approved became four when Part 2 gained the start-time wait; he has been told and has not objected.
...
## The icon workbench, the mark colors and the nav badge, 2026-09-09
...
**A row is stored for every event regardless of channel, and that reversed mid-build.** The browser preference gates only the toast. The center and the badge report everything that happened whether or not the rider wanted a popup for it.
...
**The palette went from about 80 tokens to 65**, and finding five bugs on the way is the argument for having done it: two live [#282](https://github.com/feralcreative/routeloop/issues/282) instances (`.kml-btn` and `.tb-banner` painting `$white` on `$stop`), a dark ramp whose steps 43 and 50 were both `#888888`, and two `/brand` scanner defects that made the page report 31 issue numbers as colors. `$gpx`→`$go`, `$kml`→`$stop`, `$ink-dark`→`$black`, and seven near-identical blues collapsed into `$disabled`. **Three requested merges were refused with measurements**, because they would have broken dark mode; darkening `neutral-57` took `$concrete`'s black legend from 4.66 to 4.14 and was reverted.
...
**The builder's timeline scope is a double-sided pill**, `Route | Ride`, the active side filled. Its fill is `$disabled` and not `$brand`: `$brand` is a link color the dark scheme lifts, so as a field behind white it measured 2.75:1.
...
**The role ring stopped being washed out.** Each hue takes its own maximum chroma now; all seventeen used to take the weakest hue's, which left fifteen at half strength. Equal luminance is what makes it categorical and is unchanged, so every contrast assertion still holds. `ROUTE_COLORS` was re-checked rather than re-argued and is still not an option: eleven colors for seventeen roles, and eight of the eleven fail this chart's grounds.
...
**Fuel volume arrived with a surface rather than ahead of one.** `bikes.tank_ml` mirrors `usable_range_m` exactly, and the Paddock's Tank field is what reads the preference. **`bikeInput` cannot do the whole check**—it does not know which unit the rider reads in, so it validates against the liter ceiling and a gallons rider typing 30 passes it and violates `ck_bike_tank`. `tankRefusal` runs in the route where the unit is known. Found in the browser rather than by reasoning.
...
**Reported on stage, reproduced on stage, and it was not the bug fixed an hour earlier.** Both satellites were offered gas stations in Oakland, each labeled "on their way"—which is the tell: the proposer thought VMCSC was already riding the road at Oakland.
...
**American English swept through the tree** after "car park" reached the release notes: color, neighbor, center, recognize, organize, license. `tires` stays in `place-query.js`, which is a search synonym so a rider who types it still matches.
...
Driven by hand at 1280px and 390px, light and dark, signed in against the local corpus of 17 rides. Role marks, per-role fills, record cards, the count-up, the card grid and the first-run panel all render; no console errors. Two things seen and left: a ride whose thumbnail file is absent locally leaves a gray box of the right shape (a data state on this machine, not a code path), and a long unbroken title like "Davenport/Pescadero" breaks mid-word in the dense track—which is the deliberate trade, since the alternative is a clipped title with no ellipsis reading as a different, shorter place.
