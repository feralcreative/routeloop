...
**What this branch is:** the fix for #370 above—`DIVERT_WEIGHT` and a two-pass `proposeGroupMeet` in `src/subgroups/rendezvous.ts`, `withDefaults()` closing a cap that an explicit `undefined` switched off, a Meeting points topic on Preferences posting to `/settings/meet`, the builder's dial seeded from the preference and relabeled "extra detour", and the route reading press, then preference, then default. Verified in the browser: the preference saves, clears to null and clamps 900 to 200, and the dial opens on the stored number. **Not yet pressed on a two-group ride locally**—that costs Routes and Text Search requests and there is no such ride in the dev database; the ranking itself is pinned by seven tests including epim's shape.
