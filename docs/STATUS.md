...
**STAGE IS RUNNING BLUE/GREEN as of 2026-08-27; production is not.** The rehearsal on stage is complete—six deploys, three separate zero-dropped-request measurements, Postgres never restarted. The PRODUCTION cutover is the only step in either phase that can lose a database, its runbook is in [zero-downtime-deploy.md](zero-downtime-deploy.md), and its preconditions are not optional: a backup that does not live on the deploying laptop, ground truth about the volume first, and `docker stop` + `rm` of the **app container only**—never `docker-compose down`, which takes db and the network with it. Rehearse on stage.
