...
**Branch:** `fix/meeting-point-ranking` ([#370](https://github.com/feralcreative/routeloop/issues/370), plus the retroactive [#371](https://github.com/feralcreative/routeloop/issues/371) and [#372](https://github.com/feralcreative/routeloop/issues/372))—the meeting-point proposer scores a trade instead of taking the earliest point under the cap, the allowance is measured from each group's cheapest meet rather than from zero, the default drops from 25 to 10, and a rider sets their own on `/settings`. **Migration `drizzle/0044`, additive (`user_profiles.meet_divert_mi`), so prod deploys first.** Cut from `main` on 2026-09-19 after #369 merged. **3,092 tests across 122 files** (2 skipped, 3,094 total).
...
**PROD IS ON `f017706`** (verified 2026-09-18 via `/healthz`), which is #369 merged; this branch adds migration `0044`, so `utils/deploy/both.sh` prod first when it lands.
