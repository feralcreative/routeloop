...
**Branch:** `feat/preferences-pass` ([#389](https://github.com/feralcreative/routeloop/issues/389)–[#391](https://github.com/feralcreative/routeloop/issues/391))—a pass at the Preferences tab: every choice card's sentence moved into a tooltip and a per-group `?` key, Appearance four across and Units five, the power picker following the vehicle, and the vocabulary overrides as one four-column table of default pill plus custom box. No migration. Cut from `main` on 2026-09-21 after #387 merged. **3,126 tests across 125 files** (3 skipped, 3,129 total).
...
**PROD IS BEHIND** by #373 (migration `0044`) and this branch; `utils/deploy/both.sh`, prod first.
