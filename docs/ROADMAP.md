...
- [ ] **The action row is icons, not labels.** Save / Undo / Redo / Discard changes / status is 308px of controls in a 344px row, so it already wraps to two lines at 47px tall. Icons at a fraction of the width. **Sequence this after autosave:** that item deletes the Save button outright and probably Discard with it, so the set may be down to undo, redo and a status indicator before anything gets drawn—design the row that survives, not the one that exists.
...
- **Whether exiting needs a guard at all.** A confirm dialog with a "don't show again" was the first thought, and Settings would hold the un-dismiss—which would give `/settings` its first real content and answer the open question in `docs/main-menu.md`. But autosave above may remove the reason for the dialog entirely: nothing to lose means nothing to confirm. Settle autosave first.
- **Save churns IDs.** The `PUT` deletes and re-inserts every day and point, so identifiers change on each save. Harmless today; it will bite the moment anything references a point across saves—rich stop details (item 11) or comments. Decide before autosave multiplies the frequency.
...
**Touches.** `src/routes/rides.ts` (the panel markup and `PUT /api/rides/:id`), `src/views/layout.tsx` (`panelShell`, shared with the viewer—so the collapse/close change lands on both), `public/js/builder.js`, `public/js/builder-history.js`, `style/_builder.scss`, `src/routes/settings.tsx`, `docs/ops/faq.md`.
