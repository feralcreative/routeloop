...
**The straight line could not see the bay, so the road decides the order.** Ziad drove the repro after the trade landed and got the same answer. From San Francisco the straight line to Fremont crosses the water—9.6 paper miles, 21 by road—and a group with a starting point and no route had no track for the on-route test, so nothing said Dublin was on their way. A start-only group now has its direct road to the destination fetched once (one cached Routes request per press) and used as its track, so the convergence is where their road meets the main group's; and `rankByRoad()` re-orders the routed shortlist by real miles—approach plus the trunk from there, least is the floor, extra over it is capped and weighed. Every leg is road and the trunk is billed only between candidates, which is what keeps it from being the #239 mixing. `reachable()` routes until the budget is spent rather than until three are kept, the convergence is front-loaded into the search anchors and the routing order, and stations are spread at 3 km rather than 10. On the repro: the Livermore Costco and Shell at zero, Sunol +14.8 and Fremont +21.2 behind them.

**The avoid list reaches the proposal, as a nudge** ([#372](https://github.com/feralcreative/routeloop/issues/372)). Ziad's call the same day, reversing the 2026-09-07 rule in `src/places/ranking.ts` that it must never: Costco Gas was on his avoid list and the proposal was a Costco, with a Shell four miles on. Weight, never remove—chosen over dropping avoided stations unless nothing else is left, and over a hard filter—so the settings page's "nothing is hidden" stays true and a lone avoided station on a rural stretch is still offered. `PLACE_NUDGE_MI` is 8, about five miles of extra detour or eight further along, applied in both passes.
