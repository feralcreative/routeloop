...
import { concatSplitLegs, relegRoute, splitRouteTrack } from '../src/maps/track-split'
...
describe('the concatenation is the original track', () => {
  it('reproduces it element for element', () => {
    const track = line(50)
    const out = splitRouteTrack(track, [at(track, 0), at(track, 17), at(track, 31), at(track, 49)])
  ...
  it('holds when the ends had to be invented', () => {
    const track = line(40)
    const out = splitRouteTrack(track, [at(track, 12), at(track, 27)])
  ...
  it('holds with no points at all, which is most GPX files', () => {
    const track = line(30)
    const out = splitRouteTrack(track, [])
...
describe('legs', () => {
  it('shares the joint vertex between consecutive legs', () => {
    ...
    const track = line(20)
    const { legs } = splitRouteTrack(track, [at(track, 0), at(track, 9), at(track, 19)])
  ...
  it('never emits a leg with fewer than two vertices', () => {
    ...
    const { legs } = splitRouteTrack(track, crowded)
  ...
  it('sums to the whole track within rounding', () => {
    const track = line(60)
    const { legs } = splitRouteTrack(track, [at(track, 0), at(track, 20), at(track, 40), at(track, 59)])
  ...
  it('produces exactly points - 1 legs, which is what routeSchema demands', () => {
    const track = line(40)
    for (const n of [0, 1, 2, 5, 9]) {
      const pts = Array.from({ length: n }, (_, i) => at(track, 1 + i * 3))
      const out = splitRouteTrack(track, pts)
  ...
  it('counts a POI as a boundary, so it splits the leg it sits on', () => {
    const track = line(30)
    const withoutPoi = splitRouteTrack(track, [at(track, 0), at(track, 29)])
    const withPoi = splitRouteTrack(track, [at(track, 0), poi(-122, 37.015, 'viewpoint'), at(track, 29)])
...
describe('point ordering', () => {
  it('sorts along the track, not by document order', () => {
    ...
    // the reordering.
    const out = splitRouteTrack(track, [at(track, 29, 'third'), at(track, 0, 'first'), at(track, 10, 'second')])
  ...
  // put a viewpoint from the middle of the route at the end of the list. Placing
  ...
  it('places POIs in the one ordered list, along the track like any other point', () => {
    const track = line(30)
    const out = splitRouteTrack(track, [at(track, 29, 'end'), poi(-122, 37.015, 'viewpoint'), at(track, 0, 'start')])
    expect(out.points.map((s) => s.name)).toEqual(['start', 'viewpoint', 'end'])
    expect(out.points.map((s) => s.kind)).toEqual([undefined, 'poi', undefined])
  })

  // A file of nothing but POIs would otherwise leave the route with no stop at all,
  // which routeSchema refuses — the same rule the builder applies to a route's first
  // point, applied at the import.
  it('promotes the first point when a file names no stop at all', () => {
    const track = line(30)
    const out = splitRouteTrack(track, [poi(track[0][0], track[0][1], 'a'), poi(track[29][0], track[29][1], 'b')])
...
describe('endpoints', () => {
  it('does not invent a stop when one already sits at the end', () => {
    const track = line(25)
    const out = splitRouteTrack(track, [at(track, 0), at(track, 24)])
    expect(out.synthesizedStart).toBe(false)
    expect(out.synthesizedEnd).toBe(false)
    expect(out.points).toHaveLength(2)
  })
  ...
  it('tolerates a stop near the end rather than doubling it up', () => {
    ...
    const out = splitRouteTrack(track, [nearStart, at(track, 24)])
    expect(out.synthesizedStart).toBe(false)
  ...
  it('invents one when the nearest stop is genuinely far from the end', () => {
    const track = line(60)
    const out = splitRouteTrack(track, [at(track, 30), at(track, 59)])
    expect(out.synthesizedStart).toBe(true)
    expect(out.synthesizedEnd).toBe(false)
  ...
  it('gives a single point a partner so there is a leg at all', () => {
    const track = line(20)
    const out = splitRouteTrack(track, [at(track, 0, 'Only')])
...
describe('degenerate input', () => {
  it('keeps the points and makes no legs when there is no track', () => {
    ...
    const out = splitRouteTrack([], [stop(-122, 37), stop(-121, 38)])
  ...
  it('treats a one-vertex track as no track', () => {
    const out = splitRouteTrack(line(1), [stop(-122, 37)])
    expect(out.legs).toEqual([])
  ...
  it('keeps every point when the track is too short to give each its own vertex', () => {
    ...
    const out = splitRouteTrack(track, many)
  ...
  it('survives a track carrying duplicate vertices', () => {
    ...
    const out = splitRouteTrack(dupes, [at(dupes, 0), at(dupes, 15)])
...
// The restore path for a native file written before 2026-08-24, when a route
// carried stops - 1 legs. The points are all there and already in the rider's
// order; what the file lacks is a leg per pair.
describe('re-cutting a route whose order is already right', () => {
  it('keeps the rider order rather than re-deriving it', () => {
    ...
    // rider's own choice, so sorting would rearrange a route they had arranged.
    const pts = [at(track, 0), at(track, 20), at(track, 10), at(track, 29)]
    const legs = relegRoute(track, pts)
  ...
  it('forces both ends onto the track, so nothing recorded is dropped', () => {
    ...
    const legs = relegRoute(track, [at(track, 12), at(track, 27)])
    expect(concatSplitLegs(legs)).toEqual(track)
  })
  ...
  it('gives one leg per pair, whatever the kinds are', () => {
    ...
    expect(relegRoute(track, pts)).toHaveLength(3)
  })

  it('has nothing to do with fewer than two points, or with no track', () => {
    const track = line(20)
    expect(relegRoute(track, [at(track, 0)])).toEqual([])
    expect(relegRoute([], [stop(-122, 37), stop(-121, 38)])).toEqual([])
    expect(relegRoute(line(1), [stop(-122, 37), stop(-121, 38)])).toEqual([])
  })
})
