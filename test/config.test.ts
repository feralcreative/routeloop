...
describe('isLocalDatabaseUrl', () => {
  it('accepts the three hosts a dev database actually uses', () => {
    for (const h of ['127.0.0.1', 'localhost', 'host.docker.internal']) {
      expect(isLocalDatabaseUrl(`postgres://u:p@${h}:5432/tankbag`)).toBe(true)
    }
  })

  it('rejects anything else, which is what stops a seeder truncating production', () => {
    expect(isLocalDatabaseUrl('postgres://u:p@nas.feralcreative.co:5432/tankbag')).toBe(false)
...
describe('redactDatabaseUrl', () => {
  it('hides the password in the userinfo', () => {
    expect(redactDatabaseUrl('postgres://tankbag:s3cr3t@127.0.0.1:5432/tankbag')).toBe(
      'postgres://***@127.0.0.1:5432/tankbag',
    )
  })
  ...
  it('hides a password passed as a query parameter', () => {
    ...
    expect(redactDatabaseUrl('postgres://tankbag@host/db?password=hunter2')).toBe(
  ...
  it('keeps the host visible, which is the entire reason it is printed', () => {
    ...
    expect(redactDatabaseUrl('postgres://u:p@nas.feralcreative.co/tankbag')).toContain('nas.feralcreative.co')
  })

  it('never leaves the secret in the output', () => {
    for (const u of [
      'postgres://tankbag:s3cr3t@127.0.0.1:5432/tankbag',
      'postgres://tankbag@host/db?password=s3cr3t',
      'postgres://tankbag:s3cr3t@host/db?password=s3cr3t',
    ]) {
  ...
  it('leaves a URL with nothing to hide alone', () => {
    expect(redactDatabaseUrl('postgres://127.0.0.1:5432/tankbag')).toBe('postgres://127.0.0.1:5432/tankbag')
