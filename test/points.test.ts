...
// later, so a route is one ordered array and `kind` is a flag on an element rather
...
describe('the shape of a route', () => {
  it('keeps one ordered list, both kinds interleaved as the rider left them', () => {
    ...
    expect(r.routes[0].points.map((p) => p.name)).toEqual(['A', 'View', 'B'])
    expect(r.routes[0].points.map((p) => p.kind)).toEqual(['stop', 'poi', 'stop'])
  })
  ...
  it('defaults a point with no kind to a POI', () => {
    ...
    expect(r.routes[0].points[1].kind).toBe('poi')
  })
  ...
  it('counts legs against every point, both kinds', () => {
    const ok = ridePayload.safeParse(rideWith([pt({ kind: 'stop' }), pt(), pt(), pt(), pt({ kind: 'stop' })], 4))
  ...
  it('gives a route of a stop and a POI a leg between them', () => {
    const ok = ridePayload.safeParse(rideWith([pt({ kind: 'stop' }), pt({ name: 'View' })], 1))
    expect(ok.success).toBe(true)
  })

  it('refuses a route whose legs do not connect its points', () => {
  ...
  // Still required, and no longer for a routing reason: a route of nothing but POIs
  ...
  it('refuses a route with no stops at all', () => {
...
describe('a native file written before the merge', () => {
  ...
  it('is merged into one ordered list, stops first', () => {
    const upgraded = upgradeNativeRide(legacy(3)) as { routes: Array<{ points: Array<{ name: string }> }> }
    expect(upgraded.routes[0].points.map((p) => p.name)).toEqual(['A', 'B', 'View'])
  })
  ...
  it('stamps the kinds explicitly rather than letting a stop default to a POI', () => {
    const upgraded = upgradeNativeRide(legacy(3)) as { routes: Array<{ points: Array<{ kind: string }> }> }
    expect(upgraded.routes[0].points.map((p) => p.kind)).toEqual(['stop', 'stop', 'poi'])
  })

  it('drops the old arrays so the merged list is the only one', () => {
    const upgraded = upgradeNativeRide(legacy(3)) as { routes: Array<Record<string, unknown>> }
    expect(upgraded.routes[0].stops).toBeUndefined()
    expect(upgraded.routes[0].pois).toBeUndefined()
  })
  ...
  it('validates against the current schema once upgraded', () => {
    ...
    if (parsed.success) {
      ...
      expect(parsed.data.routes[0].points).toHaveLength(3)
      expect(parsed.data.routes[0].points[2].durationMin).toBe(15)
    }
  })

  it('does the same for the legacy tankbag version key', () => {
    const upgraded = upgradeNativeRide(legacy(2, 'tankbag')) as { routes: Array<{ points: unknown[] }> }
    expect(nativeVersion(legacy(2, 'tankbag'))).toBe(2)
    expect(upgraded.routes[0].points).toHaveLength(3)
  })

  // A current file must pass through untouched — the merge is keyed on the
  // version, not on the absence of a `points` array, so a v4 route is not re-read.
  it('leaves a current file alone', () => {
    ...
    const upgraded = upgradeNativeRide(current) as { routes: Array<{ points: Array<{ name: string }> }> }
    expect(upgraded.routes[0].points.map((p) => p.name)).toEqual(['A', 'V'])
  })
})
