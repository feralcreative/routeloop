// How twisty a route's riding is, as one number derived from the route's shape.
//
...
// loops back near where it started, not whether the road bends. One route scored
// 34.15 sinuosity at 113°/mi while the twistiest route in the set scored 1.76 at
// 214°/mi.
...
// twistiest and flattest route in the dev corpus. That was the wrong test, and it
...
// The cost is that the flattest desert route in the corpus rises from 10 to 16°/mi
// — real gentle interstate bends, correctly counted rather than discarded.
const DEADBAND_DEG = 1

// The window for "the best stretch of the route". Per-route averaging buries a good
...
// on the dev corpus every single route scored between 122 and 1010, desert
...
// properly: the desert routes fall to 35-63 while genuinely twisty ones hold
// 300-493. 40 miles over-smooths, and is longer than some whole routes.
const WINDOW_MI = 20
...
//
// Calibrated against the 19 distinct routes in the dev database at the
...
// sweepers scores 226 and one at 200m scores 442, so a whole route at 250 means
// its good sections are a good deal tighter than that — a route always dilutes
// itself with transit.
//
...
// line that gets wavier with it, flat for 1 and a tight sine for 5, WITH THE
// WORD BESIDE IT (`twistScale()` in src/views/twist-scale.ts and
// public/js/twist.js, drawn by `.twist-scale[data-rank]` in _chrome.scss), and
// the dashboard's `?` shows all five (`twistKey()`). The line is what makes
// the word mean something at a glance; the word is what makes one line mean
// something on its own, which alone it did not. A sparkline, not letter
// grades, a percentage or a row of marks: a straight highway is a property
// rather than an F, a percentage of an unbounded figure means nothing, and
// five repeated glyphs read as a rating — Ziad hated them.
...
export type Twistiness = {
  /** Degrees of heading change per mile across the whole route. */
  dpm: number
  /** The same figure over the twistiest WINDOW_MI-mile stretch of it. */
  bestDpm: number
  /** How long that stretch actually was — shorter than the window on a short route. */
  bestMiles: number
}

// Exported for src/maps/expand.ts, which places shaping points by heading
// change: the same signal this file scores a route by, used to decide where a
...
export function twistiness(track: Track): Twistiness | null {
  ...
  // to the whole route when the route is shorter than the window.
